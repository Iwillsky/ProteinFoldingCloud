...
HOST = config2.settings['host']
MASTER_KEY = config2.settings['master_key']
DATABASE_ID = config2.settings['database_id']
CONTAINER_ID = config2.settings['container_id']
...
def query_jobs(container, strJobid):
    print('\nQuerying for an  Item by Jobid\n')

    # Including the partition key value of account_number in the WHERE filter results in a more efficient query
    items = list(container.query_items(        
        query="SELECT * FROM r WHERE r.state=@valJobid",
        parameters=[
            { "name":"@valJobid", "value": strJobid }
        ],               
        enable_cross_partition_query=True,
        max_item_count=1
    ))

    print('Item queried by Jobid {0}'.format(items[0].get("name")))
